    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if video_id:
                ...
                response = requests.get(f'https://www.googleapis.com/youtube/v3/videos?part=snippet&id={ video_id[0] }&key={ settings.YOUTUBE_API_KEY }').json()
...
def video_search(request):
    ...
    if search_form.is_valid():
        encodded_search_term = urllib.parse.quote(search_form.cleaned_data['search_term'])
        response = requests.get(f'https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=5&q={ encodded_search_term }&key={ settings.YOUTUBE_API_KEY }').json()
...
class UpdateHall(LoginRequiredMixin ,generic.UpdateView):
    ...
    def get_success_url(self):
        #goto detail page after updation
        return reverse_lazy('detail_hall', kwargs={'pk': self.object.pk})
